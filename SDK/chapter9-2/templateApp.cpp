...
void templateAppDraw( void )
{
    ...
    float orientation[6] = { 0.0f, 1.0f, 0.0f,  // The direction vector the
                                                // listener is looking at.
                             0.0f, 0.0f, 1.0f };// The world up vector.
    /* Send the direction vector and up vector of the listener.  In OpenAL terms,
     * this is called the "listener orientation".
     */
    alListenerfv(AL_ORIENTATION, &orientation[0]);
    /* Specify the position of the listener in world space coordinates. */
    alListener3f(AL_POSITION, 0.0f, 0.0f, 0.0f);

    /* Static variable to make the sound source position go forward on the
     * Y axis, away from the listener. */
    static float y = 0.0f;
    /* Assign the new position to the sound source. */
    alSource3f(soundsource, AL_POSITION, -2.0f, y, 0.0f);
    /* Increase the sound Y position. */
    y += 0.5f;
}
