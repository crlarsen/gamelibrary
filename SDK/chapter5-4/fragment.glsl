...
/* Receive the specular color of the material. */
uniform lowp vec3 SPECULAR_COLOR;

/* Receive the shininess of the material. */
...
void main( void )
{
    ...
    if ( intensity > 0.0 ) {
        /* Calculate the reflection vector in order to determine the
         * specular intensity of the fragment.
         */
        lowp vec3 reflectionvector = normalize( - reflect( lightdirection_ts, normal ) );

        /* Calculate the final fragment color including the specularity. */
        gl_FragColor += texture2D( DIFFUSE, texcoord0 ) * vec4( DIFFUSE_COLOR, 1.0 ) * intensity +
                        vec4( SPECULAR_COLOR, 1.0 ) * pow( max( dot( reflectionvector, position ), 0.0), SHININESS );
    }
}
